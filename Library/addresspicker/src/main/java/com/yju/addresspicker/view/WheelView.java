...
package com.yju.addresspicker.view;
...
import com.yju.addresspicker.R;
...
//					itemsPaint));
			float textWidth = Math.abs(Layout.getDesiredWidth("0",
					itemsPaint));
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int) Math.abs(Layout.getDesiredWidth(label,
					valuePaint));
		}
